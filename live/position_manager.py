...
class PositionManager:
    ...
    def can_trade(self, signal: Signal) -> Tuple[bool, str]:
        ...
        GTC 订单下单时立即 record_trade（保守计入仓位），
        超时取消成功后 reverse_trade 回退。
        所以这里只需检查 filled 仓位即可。
        ...
        with self._lock:
            ...
            current_net = pos.net_shares if pos is not None else 0
    ...
    def reverse_trade(self, signal: Signal) -> None:
        """回退已计入的仓位（GTC 订单超时取消成功时调用）"""
        with self._lock:
            pos = self._positions.get(signal.condition_id)
            if pos is None:
                logger.warning(f"reverse_trade: {signal.condition_id} 无仓位记录")
                return

            cost = signal.shares * signal.price
            if signal.direction == "YES" and signal.side == "BUY":
                pos.yes_shares = max(0, pos.yes_shares - signal.shares)
                pos.yes_cost = max(0.0, pos.yes_cost - cost)
            elif signal.direction == "NO" and signal.side == "BUY":
                pos.no_shares = max(0, pos.no_shares - signal.shares)
                pos.no_cost = max(0.0, pos.no_cost - cost)

            self._total_cost = max(0.0, self._total_cost - cost)

            logger.info(
                f"仓位回退: K={signal.strike:.0f} {signal.direction} "
                f"-{signal.shares}份, 净仓={pos.net_shares}"
            )
